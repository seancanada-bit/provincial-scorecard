...
/**
 * Generic portal tooltip.
 * Wraps a single child element — hover (desktop) or tap (mobile) reveals content.
 * Smart-positions above or below based on available viewport space.
...
export default function Tooltip({ children, content }) {
  ...
  const place = useCallback(() => {
    ...
    // Centre on trigger, clamped so card never clips viewport edge
    const HALF = 116; // half of tipcard width (232px / 2)
  ...
  // Close on outside click / touch (mobile dismiss)
  useEffect(() => {
    if (!visible) return;
    const handler = e => {
      if (
